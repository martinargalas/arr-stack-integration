...
class ArrStackProxyView(HomeAssistantView):
    ...
    async def _qbit_login(self, session: aiohttp.ClientSession) -> None:
        ...
        url = f"{self._cfg[CONF_QBIT_URL]}/api/v2/auth/login"
        async with session.post(url, data={
            "username": self._cfg[CONF_QBIT_USER],
            "password": self._cfg[CONF_QBIT_PASS],
        }) as r:
            await r.read()
    ...
    async def _route(
        ...
        # ════════════════════════════════════════════
            ...
            # Přidání filmu do Radarru (unmonitored, pro IS)
            ...
            # Historie stažených releasů pro daný film
            if path == "history" and method == "GET":
                ...
                async with http.get(
                    ...
                    headers=hdrs,
                    params={"movieId": movie_id, "includeMovie": "false", "pageSize": "50"},
                ) as r:
                    return web.Response(
                        body=await r.read(),
                        content_type="application/json",
