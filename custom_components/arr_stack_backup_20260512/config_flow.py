...
async def _test_qbit(session: aiohttp.ClientSession, url: str, user: str, password: str) -> str | None:
    """Vrátí None pokud OK, jinak popis chyby."""
    try:
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            err = await _test_qbit(
                session,
                user_input[CONF_QBIT_URL],
                user_input[CONF_QBIT_USER],
                user_input[CONF_QBIT_PASS],
            )
            if err:
                ...
                err = await _test_sabnzbd(
                    session,
                    user_input[CONF_SAB_URL],
                    user_input[CONF_SAB_KEY],
                )
                if err:
        ...
        schema = vol.Schema({
            vol.Required(CONF_QBIT_URL,  default=self._data.get(CONF_QBIT_URL,  "http://192.168.1.x:8088")): str,
            vol.Required(CONF_QBIT_USER, default=self._data.get(CONF_QBIT_USER, "admin")): str,
            vol.Required(CONF_QBIT_PASS, default=self._data.get(CONF_QBIT_PASS, "")): str,
            vol.Required(CONF_SAB_URL,   default=self._data.get(CONF_SAB_URL,   "http://192.168.1.x:8080")): str,
            vol.Required(CONF_SAB_KEY,   default=self._data.get(CONF_SAB_KEY,   "")): str,
        })
        return self.async_show_form(
            step_id="user",
            data_schema=schema,
            errors=errors,
            description_placeholders={"step": "1/3 — Stahování"},
        )

    # ── Krok 2: Radarr + Sonarr ───────────────────────────────────────────────
